...
from todo.get_model import get_model, get_model_as_str
...
class AbstractTaskList(models.Model):
...
class AbstractTask(models.Model):
    ...
    task_list = models.ForeignKey(
        get_model_as_str("TaskList"), on_delete=models.CASCADE, null=True
    )
    ...
    def merge_into(self, merge_target):
        ...
        # the cascade clause
        with LockedAtomicTransaction(get_model("Comment")):
            get_model("Comment").objects.filter(task=self).update(task=merge_target)
            self.delete()
...
class AbstractComment(models.Model):
    ...
    task = models.ForeignKey(get_model_as_str("Task"), on_delete=models.CASCADE)
